...
func setup(label_text: String, label_color: Color, world_pos: Vector3, camera: Camera3D) -> void:
	text = label_text
...
func _process(delta: float) -> void:
	...
	var font_size := int(remap(zoom, 60.0, 230.0, 48, 24))
	...
	position = screen_pos + Vector2(-size.x / 2.0, -base_offset - t * float_range)
	modulate.a = 1.0 - t
