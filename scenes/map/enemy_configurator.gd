...
func spawn(zone_id: Vector2i = Vector2i.ZERO) -> void:
	...
	for record in returning:
		...
		if not organs.is_empty():
			var cardio: Node = enemy.organs.cardiovascular
			if cardio != null:
				cardio.heart_rate = organs.get("heart_rate", cardio.heart_rate)
				cardio.monitor.bp_systolic = organs.get("bp_systolic", cardio.monitor.bp_systolic)
				cardio.monitor.bp_diastolic = organs.get("bp_diastolic", cardio.monitor.bp_diastolic)
				cardio.monitor.mean_arterial_pressure = organs.get("mean_arterial_pressure", cardio.monitor.mean_arterial_pressure)
			...
			var renal: Node = enemy.organs.renal
			if renal != null:
			...
			var coag: Node = enemy.organs.coagulation
			if coag != null:
